...
export default function CrashTimeline() {
  ...
  const availableLocations = useMemo(() => {
    ...
    if (locationType === "state") {
      ...
      return Object.entries(data.crashesByCity)
        .filter(([city, count]) => count >= 10 && !city.includes("NOT APPLICABLE"))
        .map(([city]) => city)
        .sort()
    }
  ...
  const trendAnalysis = useMemo(() => {
    if (data.loading || timelineData.length < 2) return { trend: "stable", percentage: 0 }

    const firstYear = timelineData[0]
    const lastYear = timelineData[timelineData.length - 1]

    if (firstYear.crashes === 0) return { trend: "increasing", percentage: 100 }

    const changePercentage = ((lastYear.crashes - firstYear.crashes) / firstYear.crashes) * 100
  ...
  return (
    <section id="crash-timeline" className="py-20 bg-white">
      <div className="container mx-auto px-6">
        <ScrollReveal>
          <h2 className="section-title">Crash Timeline</h2>
          <p className="section-description">Explore crash trends over time for specific locations</p>
        </ScrollReveal>
        ...
        <ScrollReveal delay={200}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 max-w-4xl mx-auto">
            ...
            <div>
              <label className="block text-sm font-medium mb-2 text-gray-700">Select {locationType}</label>
              <Select value={selectedLocation} onValueChange={setSelectedLocation}>
                <SelectTrigger className="border-blue-200 focus:border-blue-500 focus:ring-blue-500 h-12">
                  <SelectValue placeholder={`Select ${locationType}`} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All {locationType === "state" ? "States" : "Cities"}</SelectItem>
        ...
        <div className="grid grid-cols-1 gap-8">
            ...
            <Card className="chart-container">
              <CardHeader>
                <CardTitle className="text-xl text-gray-800">
                  Yearly Crash Trend {selectedLocation !== "all" ? `for ${selectedLocation}` : ""}
                </CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <p className="text-gray-700">
                      ...
                      {trendAnalysis.trend === "increasing"
                        ...
                        : trendAnalysis.trend === "decreasing"
                          ? " decrease"
                          : " change"}{" "}
                      from {timelineData[0].year} to {timelineData[timelineData.length - 1].year}.
                      {trendAnalysis.trend === "increasing" &&
          ...
          <ScrollReveal delay={400}>
            <Card className="chart-container">
              <CardHeader>
                <CardTitle className="text-xl text-gray-800">
                  Monthly Crash Distribution {selectedLocation !== "all" ? `for ${selectedLocation}` : ""}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={monthlyTrendData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-gray-700">
                    The data reveals distinct seasonal patterns in crash frequencies. Winter months often show increased
                    crashes due to adverse weather conditions, while summer months may see higher crash rates due to
                    increased travel and tourism. Understanding these seasonal variations can help authorities allocate
